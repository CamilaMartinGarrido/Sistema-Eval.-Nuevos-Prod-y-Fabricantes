...
CONSTRAINT chk_eval_process_dates CHECK (
    (archive_date IS NULL) OR 
...
COMMENT ON COLUMN evaluation_process.created_at IS 'Fecha y hora de creación del registro.';
COMMENT ON COLUMN evaluation_process.updated_at IS 'Fecha y hora de última actualización del registro.';
...
CREATE INDEX idx_evaluation_process_created ON evaluation_process (created_at);
CREATE INDEX idx_evaluation_process_archived ON evaluation_process (archive_date) WHERE lifecycle_state = 'Archivado';

-- Índice compuesto para búsquedas frecuentes
CREATE INDEX idx_eval_process_app_state_date ON evaluation_process (application_id, lifecycle_state, created_at);
...
-- PROCEDIMIENTO PARA ARCHIVAR SOLICITUD (CON ARCHIVOS DE PROCESOS)
CREATE OR REPLACE PROCEDURE archive_application(
    p_application_id INTEGER,
    p_reason archive_reason_enum DEFAULT NULL,
    p_archive_processes BOOLEAN DEFAULT TRUE  -- Nuevo parámetro para controlar
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    IF p_reason IS NULL THEN
        -- Intentar determinar la razón automáticamente
        IF EXISTS (
    ...
    BEGIN
        ...
        IF p_archive_processes THEN
            IF v_has_lifecycle_state THEN
                -- Opción A: evaluation_process tiene columna lifecycle_state
            ...
            ELSE
                -- Opción B: evaluation_process NO tiene columna lifecycle_state
                -- Solo podemos contar los procesos, no archivarlos
...
COMMENT ON PROCEDURE archive_application IS 'Archiva una solicitud y opcionalmente sus procesos de evaluación asociados, con validaciones y trazabilidad. Parámetro p_archive_processes controla si archivar procesos.';
...
COMMENT ON COLUMN sample_analysis.analyst_name IS 'Nombre del analista que realizó el análisis.';
...
-- Función de validación mejorada para la nueva estructura
...
DECLARE
    ...
    v_process_filter TEXT;
BEGIN
    -- Determinar filtro según si es para proceso específico o global
    IF p_evaluation_process_id IS NOT NULL THEN
        v_process_filter := ' AND ep.id = ' || p_evaluation_process_id::TEXT;
    ELSE
        v_process_filter := '';
    END IF;
    
    -- 1. Verificar oferta exploratoria competitiva (PARA ESTE PROCESO)
    EXECUTE format('
        SELECT EXISTS (
    ...
    USING p_maker_product_id
    INTO v_has_competitive_offer;
    
    IF NOT v_has_competitive_offer THEN
        -- Verificar si hay oferta no competitiva para este proceso
        EXECUTE format('
            SELECT EXISTS (
                SELECT 1 FROM exploratory_offer eo
                JOIN evaluation_process ep ON ep.id = eo.evaluation_process_id
        ...
        IF v_has_competitive_offer THEN
            RETURN 'No Aprobado';
        ELSE
    ...
    -- 2. Verificar documentos técnicos (PARA ESTE PROCESO)
    EXECUTE format('
        SELECT 
            EXISTS (
            ...
            NOT EXISTS (
                ...
                JOIN evaluation_process ep ON ep.supply_id = s.id
                WHERE mp.id = $1 %s
                  AND (de.is_approved = false OR de.is_approved IS NULL)
                  AND de.evaluation_date IS NOT NULL
            )', v_process_filter, v_process_filter)
    USING p_maker_product_id
    INTO v_has_documents_requested, v_documents_approved;
    
    IF NOT v_has_documents_requested THEN
        RETURN 'Pendiente de Documentación';
    END IF;
    
    IF NOT v_documents_approved THEN
        -- Si hay documentos evaluados y no aprobados
        EXECUTE format('
            SELECT EXISTS (
                SELECT 1 FROM document_evaluation de
                JOIN technical_document td ON td.id = de.technical_document_id
        ...
        IF v_has_documents_requested THEN
            RETURN 'No Aprobado';
        ELSE
            RETURN 'Pendiente de Documentación';
        END IF;
    ...
    -- 3. Verificar muestras (PARA ESTE PROCESO)
    EXECUTE format('
        SELECT 
            EXISTS (
                SELECT 1 FROM sample s
                JOIN supply sup ON sup.id = s.supply_id
                JOIN maker_product mp ON mp.id = sup.maker_product_id
                WHERE mp.id = $1 %s
                  AND s.request_date IS NOT NULL
                  AND EXISTS (
                      SELECT 1 FROM evaluation_process ep 
                      WHERE ep.supply_id = sup.id 
                        AND ep.id = $2
                  )
            ),
            NOT EXISTS (
                ...
                JOIN maker_product mp ON mp.id = s.maker_product_id
                WHERE mp.id = $1 %s
                  AND se.evaluation_process_id = $2
                  AND se.result = ''No Conforme''
                  AND se.evaluation_date IS NOT NULL
            )', v_process_filter)
    USING p_maker_product_id, p_evaluation_process_id
    INTO v_has_samples_requested, v_samples_conforme;
    
    IF NOT v_has_samples_requested THEN
        RETURN 'Pendiente de Muestra';
    END IF;
    
    IF NOT v_samples_conforme THEN
        -- Si hay muestras evaluadas y no conformes
        EXECUTE format('
            SELECT EXISTS (
                SELECT 1 FROM sample_evaluation se
                JOIN sample_analysis sa ON sa.id = se.sample_analysis_id
        ...
        IF v_has_samples_requested THEN
            RETURN 'No Aprobado';
        ELSE
            RETURN 'Pendiente de Muestra';
        END IF;
    ...
    -- 4. Verificar decisión de continuar a escala industrial (PARA ESTE PROCESO)
    EXECUTE format('
        SELECT EXISTS (
            SELECT 1 FROM sample_evaluation se
            JOIN sample_analysis sa ON sa.id = se.sample_analysis_id
    ...
    IF NOT v_decision_continue THEN
        -- Verificar si ya tomó la decisión y fue negativa
        EXECUTE format('
            SELECT EXISTS (
                SELECT 1 FROM sample_evaluation se
                JOIN sample_analysis sa ON sa.id = se.sample_analysis_id
                ...
                JOIN evaluation_process ep ON ep.supply_id = s.id
                WHERE mp.id = $1 %s
                  AND se.decision_continue = false
            )', v_process_filter)
        USING p_maker_product_id
        INTO v_decision_continue;
        
        IF v_decision_continue THEN
            RETURN 'No Aprobado';
        ELSE
    ...
    -- 5. Verificar compra industrial (PARA ESTE PROCESO)
    EXECUTE format('
        SELECT 
            EXISTS (
            ...
            COALESCE(
                (SELECT purchase_status FROM industrial_purchase ip
                 JOIN evaluation_process ep ON ep.id = ip.evaluation_process_id
                 JOIN supply s ON s.id = ep.supply_id
                 JOIN maker_product mp ON mp.id = s.maker_product_id
                 WHERE mp.id = $1 %s
    ...
    USING p_maker_product_id, p_maker_product_id
    ...
    -- 6. Verificar evaluación industrial (PARA ESTE PROCESO)
    EXECUTE format('
        SELECT 
            EXISTS (
            ...
            COALESCE(
                (SELECT analysis_result FROM industrial_evaluation ie
                 JOIN industrial_purchase ip ON ip.id = ie.industrial_purchase_id
                 JOIN evaluation_process ep ON ep.id = ip.evaluation_process_id
                 JOIN supply s ON s.id = ep.supply_id
                 JOIN maker_product mp ON mp.id = s.maker_product_id
    ...
    -- 7. Verificar informe entregado (PARA ESTE PROCESO)
    EXECUTE format('
        SELECT EXISTS (
            SELECT 1 FROM industrial_evaluation ie
            JOIN industrial_purchase ip ON ip.id = ie.industrial_purchase_id
    ...
    -- 8. Resultado final basado en evaluación industrial
...
COMMENT ON FUNCTION determine_manufacturer_status IS 'Determina el estado actual de un fabricante-producto, considerando un proceso específico o todos los procesos (global).';
...
BEGIN
    ...
    INTO v_process_states, v_final_states, v_approved_count, 
...
CREATE OR REPLACE FUNCTION update_manufacturer_status_with_history(
    p_maker_product_id INTEGER,
    p_evaluation_process_id INTEGER DEFAULT NULL,
    p_new_status evaluation_state_manufacturer_enum
)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    SELECT id, evaluation_state 
    INTO v_current_record_id, v_current_status
    ...
    IF v_current_status = p_new_status THEN
        RAISE NOTICE 'Estado no ha cambiado (%)', p_new_status;
    ...
    IF v_current_record_id IS NOT NULL THEN
        ...
        RAISE NOTICE 'Cerrado registro % (estado: % -> % para %)', 
    ...
    -- 6. Registrar observación automática del cambio (opcional)
    -- (Comentado para no crear dependencia con tabla observation)
    /*
    INSERT INTO observation (observation_text, observation_date)
    VALUES (
        'Cambio automático de estado: ' || 
        COALESCE(v_current_status::text, 'Nuevo') || ' → ' || p_new_status::text ||
...
COMMENT ON FUNCTION update_manufacturer_status_with_history IS 'Actualiza el estado manteniendo historial completo. Cierra registro anterior y crea nuevo.';


-- DISPARADORES PARA ACTUALIZACIÓN AUTOMÁTICA
-- ========================================================
...
BEGIN
    ...
    ELSIF TG_TABLE_NAME = 'technical_document' THEN
        ...
        JOIN evaluation_process ep ON ep.supply_id = s.id
        WHERE s.id = NEW.supply_id
        LIMIT 1;  -- Tomar el primer proceso (debería haber solo uno)
    ...
    ELSIF TG_TABLE_NAME = 'sample' THEN
        -- Obtener del supply
        SELECT mp.id, ep.id INTO v_maker_product_id
        FROM supply s
        JOIN maker_product mp ON mp.id = s.maker_product_id
        WHERE s.id = NEW.supply_id;

        -- Obtener evaluation_process_id relacionado
...
-- PROCEDIMIENTO PARA ACTUALIZACIÓN MANUAL
-- ========================================================
...
BEGIN
    ...
    -- Definir estados permitidos para actualización manual según rol
    IF v_user_role = 'Administrador' THEN
        ...
        END IF;
    ELSIF v_user_role = 'Analista de Precios' THEN
        v_allowed_statuses := ARRAY[
            'Pendiente de Evaluación'
        ]::evaluation_state_manufacturer_enum[];
    ELSE
        RAISE EXCEPTION 'Usuario no autorizado para actualizaciones manuales';
    END IF;
    ...
    IF p_new_status != ALL(v_allowed_statuses) THEN
        RAISE EXCEPTION 'Estado % no permitido para actualización manual por este usuario', p_new_status;
    END IF;
    ...
    -- Registrar observación si se proporcionó razón
    IF p_reason IS NOT NULL THEN
        ...
        IF v_application_id IS NOT NULL THEN
            INSERT INTO observation (observation_text, observation_date)
            VALUES (
                'Actualización manual de estado por usuario ' || p_user_id || ': ' || p_reason || 
                ' (Estado anterior: ' || v_current_status || ', Nuevo estado: ' || p_new_status || 
                CASE WHEN p_evaluation_process_id IS NOT NULL 
                     THEN ', Proceso: ' || p_evaluation_process_id 
                     ELSE ', Estado global' 
                END || ')',
...
COMMENT ON PROCEDURE update_manufacturer_status_manual IS 'Permite actualizaciones manuales controladas del estado del fabricante por usuarios autorizados.';
...
CREATE OR REPLACE FUNCTION get_manufacturer_status_report(
    ...
    p_tipo_estado VARCHAR DEFAULT 'TODOS'  -- 'GLOBAL', 'PROCESO', 'TODOS'
)
RETURNS TABLE (
...
-- Índices para búsquedas por supply_id (nueva relación clave)
CREATE INDEX IF NOT EXISTS idx_sample_supply_id ON sample(supply_id);
CREATE INDEX IF NOT EXISTS idx_evaluation_process_supply_id ON evaluation_process(supply_id);

-- Índice compuesto para búsquedas frecuentes
CREATE INDEX IF NOT EXISTS idx_sample_evaluation_process_supply 
ON sample_evaluation(evaluation_process_id, sample_analysis_id);

-- Índice para validaciones rápidas
CREATE INDEX IF NOT EXISTS idx_sample_analysis_sample_id ON sample_analysis(sample_id);
CREATE INDEX IF NOT EXISTS idx_technical_document_supply_id ON technical_document(supply_id);
